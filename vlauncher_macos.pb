...
Define.s customLaunchArgumentsDefault = "-XstartOnFirstThread -Xss1M -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M"
Define.s customOldLaunchArgumentsDefault = "-XstartOnFirstThread -XX:+UseConcMarkSweepGC -XX:+CMSIncrementalMode -XX:-UseAdaptiveSizePolicy -Xmn128M"
...
Procedure.s parseLibraries(clientVersion.s, prepareForDownload.i = 0)
  ...
  Protected.i jsonRulesMember, jsonRulesOsMember, jsonNativesOsxMember
  Protected.i i, k
      ...
      If jsonRulesMember
        For k = 0 To JSONArraySize(jsonRulesMember) - 1
          ...
          If GetJSONString(GetJSONMember(GetJSONElement(jsonRulesMember, k), "action")) = "allow"
            If jsonRulesOsName <> "empty" And jsonRulesOsName <> "osx"
              allowLib = 0
            EndIf
          Else
            If jsonRulesOsName = "osx"
              allowLib = 0
            EndIf
          EndIf
        Next
        ...
        If prepareForDownload = 1
          ...
          If jsonDownloadsMember
            ...
            If jsonClassifiersMember
              jsonNativesLinuxMember = GetJSONMember(jsonClassifiersMember, "natives-macos")
              jsonNativesOsxMember = GetJSONMember(jsonClassifiersMember, "natives-osx")
              ...
              If jsonNativesLinuxMember
                ...
                libName + "-natives-macos"
              ElseIf jsonNativesOsxMember
                url = GetJSONString(GetJSONMember(jsonNativesOsxMember, "url"))
                fileSize = GetJSONInteger(GetJSONMember(jsonNativesOsxMember, "size"))
